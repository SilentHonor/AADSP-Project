...
typedef float HAOS_BrickBuffer_t[BRICK_SIZE];
...
// the global TEMPBUFFER which is accessed through the public TEMPBUFFER_PTR array
...
void* HAOS_getMcvPointer(int moduleID)
{
	...
	for (mb = masterModuleTable.begin(), me = masterModuleTable.end(); mb != me; ++mb)
	{
...
static void run()
{
	...
	//mcv se setuje na default vrendosti, posle toga readPrekickConfigs setuje na one koje su u cfg fajlu
...
static void readCfgFile(const std::string pathName, HAOS_HostCommQ_t& message)
{
	std::ifstream is;
...
static bool openFile()
{
	...
	if (!outputPath.empty())
	{
		...
		outputHandle = cl_wavwrite_open(const_cast<char*>(outputPath.c_str()), bitsPerSample, nOutChannels, sampleRate);
...
static void writeToFile()
{
	...
	// if valid channel mask is changed figure out nOutChannels based on new valid channel mask
	if (validChMask != __HAOS_PPM_VALID_CHANNELS)
	{
		...
		cl_wavwrite_update_wave_header(outputHandle, bitsPerSample, nOutChannels, outputSampleRate);
	}
	...
	for (int sample = 0; sample < samplesToWrite; sample++)
	{
		...
		{
			sampleToWrite = __HAOS_IOBUFFER_PTRS[channel][sample] * SAMPLE_SCALE ;
			cl_wavwrite_sendsample(outputHandle, sampleToWrite);
		}
	}
}
//==============================================================================
...
static bool flushFrameToFile()
{
	...
	cl_wavwrite_close(outputHandle);
	outputHandle = cl_wavwrite_reopen(const_cast<char*>(outputPath.c_str()), sampleNumber);

	if (outputHandle == NULL)
	{
		std::cerr << "Unable to open output file <flush> '" << outputPath << "'" << std::endl;
		return false;
	}
...
void HAOS_init(int argc, const char* argv[])
{
	...
	// make sure the PCM decoder module is first if other decoder is not specified
...
int* HAOS_fillInputFIFO0(int* writePtr)
{
	...
	if (!inputPath.empty())
	{
		if(firstOpen)
		{
			...
			if (inputHandle == NULL)
			{
				std::cerr << "Unable to open input file '" << inputPath << "': file does not exist or is not a WAV file" << std::endl;
			}else
			{
		...
		{
			int currSample = 0;
			int samplesToRead = FIFO0_SIZE<(samplesPerChannel*nInputChannels)?FIFO0_SIZE:(samplesPerChannel*nInputChannels);
			// read one sample
...
int* HAOS_fillInputFIFO0(int* writePtr)
{
	if(!inputPath.empty())
	{
		if(firstOpen)
		{
			readFile = fopen(const_cast<char *>(inputPath.c_str()), "rb");
			if (readFile == NULL)
				std::cerr << "Unable to open input file ' " << inputPath << " ': file does not exist!" << std::endl;
			else
			{
				// input file is already opened
				firstOpen = false;
			}
		}

		for (int i = 0; i < FIFO0_SIZE; i++)
		{
			if (!feof(readFile))
				inputFIFO0[i] = decodeSample(readFile);
			else
			{
				__HAOS_AT_EOF = true;
				inputFIFO0[i] = 0;
...
int decodeSample(FILE* rdFile)
{
	...
	//retValue = 0;
	int checkBytes = fread(ucData, sizeof(char), 1, rdFile);
	if (checkBytes < 0)
	{
		__HAOS_AT_EOF = true;
		retValue = 0;
